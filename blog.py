...
1.  **분량**: **최소 1500단어 이상**의 매우 상세하고 깊이 있는 내용.
2.  **제목 (H1)**: 키워드를 포함하여 독자의 시선을 끄는 매력적인 제목 1개.
3.  **도입부**:
    - 독자의 문제/관심사를 언급하며 공감대 형성.
    - 이 글을 통해 무엇을 얻을 수 있는지 명확히 제시.
    - '{keyword}' 키워드를 자연스럽게 1회 포함.

4.  **핵심 추천 (TOP 5-7)**:
    - 5개에서 7개의 실제 제품/서비스를 추천합니다. 가상의 'XX', 'YY'는 절대 사용 불가.
    - **각 추천 항목별 상세 분석 (필수!)**:
        - **(1) 제품/서비스명 (H3)**: 명확한 소제목.
        - **(2) 핵심 요약**: 1-2 문장으로 된 핵심 특징 요약.
        - **(3) 상세 스펙/정보**: 주요 사양, 가격, 조건 등을 **구체적인 수치**로 제시. (예: DPI, 무게, 연회비, 할인율 등)
        - **(4) 장점 (Pros)**: 이 제품/서비스를 선택해야 하는 이유 3가지 이상을 상세히 서술.
        - **(5) 단점 (Cons)**: 구매 전 고려해야 할 단점이나 아쉬운 점 2가지 이상을 솔직하게 서술.
        - **(6) 추천 대상**: "이런 분들에게 추천합니다" 형식으로, 어떤 사용자에게 가장 적합한지 명확히 제시.
    - **비교 테이블**: 추천된 모든 항목을 한눈에 비교할 수 있는 **상세한 비교표(HTML `<table>` 태그 사용)**를 반드시 포함하세요. (항목, 핵심 스펙, 가격, 평점 등)
...
- **HTML 태그 사용 필수**:
    - 모든 제목, 소제목, 항목 구분은 반드시 <h2>, <h3>, <h4> 등 **HTML 태그만 사용**하세요. **마크다운(#, ##, ### 등)은 절대 사용하지 마세요.**
    - 비교표는 <table> 태그와 <thead>, <tbody>, <tr>, <th>, <td> 등 **HTML 표 태그만 사용**하고, <th>에는 `background-color: #f5f5f5; font-weight: bold;` 스타일을, <td>에는 `border: 1px solid #ddd; padding: 8px;` 스타일을 적용해 시각적으로 보기 좋게 만드세요.

- **키워드 사용**:
    - 위에서 강조했듯이, 핵심 키워드는 **정확히 7번**만 사용하고, 제목, 도입부, 소제목, 본문, 마무리에 걸쳐 자연스럽게 분산시켜 주세요.
    - 나머지 부분에서는 의미가 같은 동의어, 유의어, 관련어를 적극적으로 활용하여 반복을 피하고 자연스러운 글의 흐름을 유지하세요. (예: '레이저 마우스' -> '고정밀 포인팅 장치', '게이밍용 마우스', '작업용 입력 도구' 등)
    - **키워드 스터핑 절대 금지**: 같은 문단/문장에 키워드를 반복하지 마세요. 키워드 밀도는 1-2% 미만으로 엄격히 유지합니다.
...
- **문체 및 서식**:
    ...
    - **국내 데이터 우선**: 한국 시장에서 실제로 사용 가능한 제품/서비스를 우선적으로 다루세요.
